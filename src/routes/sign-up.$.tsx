...
// Catch-all route (/sign-up/$) so Clerk's multi-step + verification sub-paths resolve.
...
function SignUpPage() {
  return (
    <AuthShell
      ...
      subtitle="Sign up, then connect your WyrmKeep API key to start auditing."
      footer={
        <>
    ...
    >
      {isClerkEnabled ? (
        <>
          <ClerkLoading>
            <AuthLoading label="Loading sign-up..." />
          </ClerkLoading>
          <ClerkFailed>
            <AuthProblem />
          </ClerkFailed>
...
function AuthProblem() {
  return (
    <div className="auth-status-card auth-status-card-danger">
      <p>
        Clerk could not load the sign-up form. Check that your publishable key is
